...
const KEYWORDS = {
  ...
  url: /홈페이지|사이트|웹사이트|website|homepage|\burl\b|주소.*url|url.*주소/,
...
export function mapFields(inventory, profile, target = {}) {
  ...
  const score = (key, f) => {
    ...
    // "담당자 연락처", "담당자 이메일" 처럼 담당자 + 전화/이메일 조합은 name 이 아니라 phone/email
    if (key === "name") {
      const combined = `${s.nameId} ${s.label} ${s.ph}`;
      if (KEYWORDS.phone.test(combined) || KEYWORDS.email.test(combined)) return 0;
    }
    // 이메일·URL 칸을 message 로 잡는 오탐 방지
    if (key === "message" && (f.type === "email" || f.type === "url")) return 0;
  ...
  const textKeys = ["company", "name", "phone", "email", "url", "subject", "message"];
  const candidates = {};
  ...
  // 같은 name 속성을 가진 체크박스 수 집계 → 2개 이상이면 "다중 선택 그룹" (서비스 선택 등)
  const cbFields = fields.filter((x) => x.kind === "checkbox-radix" || x.kind === "checkbox-native");
  const nameCount = {};
  for (const f of cbFields) {
    if (f.name) nameCount[f.name] = (nameCount[f.name] ?? 0) + 1;
  }
  ...
  for (const f of cbFields) {
    ...
    if (!key) {
      const isMultiGroup = f.name && (nameCount[f.name] ?? 0) > 1;
      if (f.required && !isMultiGroup) {
        unresolved.push({
          key: "checkbox",
        ...
        const hint = (f.label || f.nearby).slice(0, 30);
        const isMultiGroup = f.name && (nameCount[f.name] ?? 0) > 1;
        if (isMultiGroup) {
          warnings.push(
            `다중선택 그룹 체크박스 손대지 않음: "${hint}" — 특정 항목 선택이 필요하면 targets.json 에 "checkConsents": ["${hint}"]`,
          );
        } else {
          warnings.push(
            `체크박스 손대지 않음(필수인지 불명): "${hint}" — 체크가 필요하면 targets.json 에 "checkConsents": ["${hint}"]`,
          );
        }
      }
      continue;
  ...
  for (const f of fields) {
    ...
    // 다중선택 그룹 체크박스는 step 3에서 경고로 처리됨 — 여기서 중복 no-mapping 방지
    const isCb = f.kind === "checkbox-native" || f.kind === "checkbox-radix";
    if (isCb && f.name && (nameCount[f.name] ?? 0) > 1) continue;
    if (f.required) {
      unresolved.push({
