...
async function applyStep(page, step) {
  ...
  // 커스텀 스타일 체크박스는 input 자체가 CSS 로 숨겨져 있어 "visible" 상태가 되지 않는다.
  // 체크박스는 DOM 에 붙어있는지만 확인하고, 실제 토글은 label-click 경로로 한다.
  if (step.kind === "checkbox-native" || step.kind === "checkbox-radix") {
  ...
  switch (step.kind) {
    ...
    case "textarea": {
      ...
      const elType = await loc.getAttribute("type").catch(() => "");
      if (elType === "number") {
        // input[type=number]: fill() 이 비숫자 문자(하이픈 등)를 거부한다.
        // 숫자만 추출해 채우고, 숫자가 전혀 없으면 evaluate 로 직접 설정한다.
        const numericVal = val.replace(/[^\d.]/g, "");
        if (numericVal) {
          await loc.fill(numericVal);
        } else {
